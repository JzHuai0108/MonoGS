...
def get_points_from_depth(fov_camera, depth, scale=1):
    """
    Get 3D points in the world frame, from depth map
    """
    ...
    T_W2C_transposed = fov_camera.world_view_transform_updated
    R_C2W = T_W2C_transposed[:3, :3].clone().float().cuda()
    p_W2C = T_W2C_transposed[3, :3].clone().float().cuda()
    pts = (pts - p_W2C) @ R_C2W.transpose(-1, -2)
    return pts
